...
class Auth(object):
    ...
    def loginWithCredential(self, _id, passwd, certificate=None, systemName=None, appName=None, keepLoggedIn=True):
        ...
        if appName == 0:
            ...
            appName=appName=self.server.APP_NAME
    ...
    def loginWithQrCode(self, keepLoggedIn=True, systemName=None, appName=None, showQr=False, logNum=None, callback=None):
        if systemName is None:
            systemName="Tyfe's Operation"
        appChoose = appName
        if appName == 0:
            appName="DESKTOPMAC\t10.10.2-YOSEMITE-x64\tMAC 4.5.0"
        elif appName == 1:
        ...
        if logNum == 0:
            self.callback.QrUrl('line://au/q/' + qrCode.verifier, showQr, pointer=appChoose, logNum=logNum)
        else:
            if appChoose == 0:
                callback('line://au/q/' + qrCode.verifier, pointer=0)
            elif appChoose == 1:
                callback('line://au/q/' + qrCode.verifier, pointer=1)
