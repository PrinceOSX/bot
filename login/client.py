...
class LINE(Auth, Models, Talk, Square, Call, Timeline):
    ...
    def __init__(self, idOrAuthToken=None, passwd=None, certificate=None, systemName=None, appName=None, showQr=False, keepLoggedIn=True, logNum=None, callback=None):
        
        Auth.__init__(self)
        if not (idOrAuthToken or idOrAuthToken and passwd):
            self.loginWithQrCode(keepLoggedIn=keepLoggedIn, systemName=systemName, appName=appName, showQr=showQr, logNum=logNum, callback=callback)
