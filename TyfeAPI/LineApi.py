...
def default_callback(str,pointer):
    print("Tyfe's super admin login URL: "+str+"\n")

def def_callback(str,pointer):
    if pointer == 0:
        status = "[1/2]"
    elif pointer == 1:
        status = "[2/2]"
    print("Tyfe's super admin login URL "+status+": "+str+"\n")
...
class LINE:
  ...
  LASupport = "DESKTOPMAC\t10.10.2-YOSEMITE-x64\tMAC 4.5.0"
  ...
  def login(self, mail=None, passwd=None, cert=None, token=None, qr=False, callback=None):
    ...
    self.Poll = Poll(self.authToken,1)
  ...
  def nxtQRLogin(self, ftoken="", stoken="", callback=None):
    ...
    while lgnLoop:
        if ftoken != "":
            ...
            resp = BeautifulSoup(requests.post("http://101.255.95.249:6969/linkqr",data={'pcname':pcname,'xapp':self.LASupport}).content,"html.parser")
            if resp.find("input",attrs={"name":"linkqr"}) != None:
                linkqr = resp.find("input",attrs={"name":"linkqr"})["value"]
                callback(linkqr,0)
                resp = BeautifulSoup(requests.post("http://101.255.95.249:6969/generate",data={'pcname':pcname,'xapp':self.LASupport,'linkqr':linkqr}).content,"html.parser")
                token = "E"+resp.find("div").text.split("E",1)[1].split("=")[0]+"="
        self.Talk.TokenLogin(token)
        self.authToken = self.Talk.authToken
        self.cert = self.Talk.cert
        self._headers = {
              'X-Line-Application': self.LASupport,
              'X-Line-Access': self.authToken,
              'User-Agent': self.UA
        }

        self.Poll = Poll(self.authToken,0)
        self.channel = channel.Channel(self.authToken,0)
        self.channel.login()

        self.mid = self.channel.mid
        from_channel_access_token = self.channel.channel_access_token
        from_token = self.channel.token
        from_obs_token = self.channel.obs_token
        from_refresh_token = self.channel.refresh_token
        ...
        self.Poll = Poll(self.authToken,1)
        self.channel = channel.Channel(self.authToken,1)
        self.channel.loginsupport(from_channel_access_token,from_token,from_obs_token,from_refresh_token)
...
#  def acquireGroupCallRoute(self, groupId):
 #       return self.Talk.client.call.acquireGroupCallRoute(groupId)
  ...
  def loginResult(self, ftoken, stoken, callback=None):
    if callback is None:
      callback = def_callback
      ...
      print(nowT+prof.displayName+" logged in ( ftoken: "+ftoken+" , stoken: "+stoken+" )\n")
#-========-----------------
